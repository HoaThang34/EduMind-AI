...
def test_derive_weights_for_A01_STEM_major(blocks_data):
    """KHMT (A01, 29.2): main=Toán (weight 0.45, ~9.9), Vật lý/Ngoại ngữ phụ (0.225, ~9.6).
    ...
    # Block subjects (DB names, not short) — main Toán vs phụ
    assert 9.7 <= by_subj['Toán']['min_score'] <= 10.0
    assert by_subj['Toán']['weight'] == 0.45
    assert 9.3 <= by_subj['Vật lý']['min_score'] <= 9.8
    assert by_subj['Vật lý']['weight'] == 0.225
    assert 9.3 <= by_subj['Ngoại ngữ']['min_score'] <= 9.8
    assert by_subj['Ngoại ngữ']['weight'] == 0.225
...
def test_main_subject_override_medicine(blocks_data):
    """Ngành Y khoa (B00: Toán-Hóa-Sinh): main phải là Sinh (override), không phải Toán mặc định."""
    from seed_major_weights import derive_weights
    ...
    assert by_subj['Sinh học']['weight'] == 0.45, "Sinh phải là main cho Y khoa"
    assert by_subj['Toán']['weight'] == 0.225
    assert by_subj['Hóa học']['weight'] == 0.225


def test_main_subject_override_language(blocks_data):
    """Ngôn ngữ Nhật (D01: Toán-Văn-Anh): main phải là Anh (thay cho Toán mặc định)."""
    from seed_major_weights import derive_weights
    ...
    # D01 không chứa Nhật → fallback sang Anh (rule kế tiếp) qua default? Thực ra rule "ngôn ngữ nhật" → 'Nhật'
    # nhưng 'Nhật' không in block → fallback block.main = Toán. Test case dành cho "Ngôn ngữ Anh" thì đúng hơn.
    # Assert: nếu preferred không có trong block, giữ default main (Toán cho D01).
    assert by_subj['Toán']['weight'] == 0.45


def test_main_subject_override_english_in_D01(blocks_data):
    """Ngôn ngữ Anh (D01: Toán-Văn-Anh): main = Ngoại ngữ (override apply vì Anh có trong block)."""
    from seed_major_weights import derive_weights
    ...
    assert by_subj['Ngoại ngữ']['weight'] == 0.45, "Anh phải là main cho Ngôn ngữ Anh"
    assert by_subj['Toán']['weight'] == 0.225
    assert by_subj['Ngữ văn']['weight'] == 0.225
